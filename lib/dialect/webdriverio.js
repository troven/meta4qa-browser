...
var Apigeek = require("apigeek-dialect"), http = Apigeek.helpers.http, vars = Apigeek.helpers.vars;
...
var self = module.exports = function(learn, config) {
    ...
    learn.given(["I use $browser browser"], function(browser, done) {
        ...
        var browser = this.vars.browser = webdriverio.remote(options);
    ...
    learn.when(["I see page contains $title"], function(title, done) {
        var browser = this.vars.browser;
        assert(browser, "Not using a browser");
        ...
        browser.getTitle().then(function (page_title) {
            ...
            assert( matched, "Page does not contain "+title);
            return matched;
        }).call(done);
...
//        file = config.files+"/"+file;
    ...
    learn.then(["I stop using browser"], function(done) {
        var browser = this.vars.browser;
        assert(browser, "Not using a browser");
        browser.end();
