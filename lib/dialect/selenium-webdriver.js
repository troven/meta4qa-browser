...
module.exports = function(learn, config) {
    ...
    try {
        webdriver = require('selenium-webdriver'), By = webdriver.By, until = webdriver.until;
    } catch(e) {
        return false;
    }
    ...
    learn.given(["I use $browser browser"], function(browser, done) {
        ...
        if (config.webdriver.host && !config.webdriver.disabled) {
            console.log("Web Driver: %j", config.webdriver);
            builder.usingServer(config.webdriver.host)
        }
...
//            return false;
//        }
        driver.wait(condition, config.webdriver.timeout || 10000 );
